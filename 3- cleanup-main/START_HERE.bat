...
echo     * Preserve text regions from all processing
...
echo   - Preserving text regions from processing (--preserve-text)
echo   - GPU acceleration enabled if available (--use-gpu)
echo.

python restore_playmat_hsv.py scans/ --skip-outline-normalization --skip-despec --use-natural-green --skip-infill --preserve-text --use-gpu
if errorlevel 1 (
    echo.
